# Unity 2019 Universal Fix

This is just a template to get started with the 2019 edition of the universal fix. There are no real changes to the configuration, just bringing it forward with the latest `d3dx.ini` with geo-11.

You must still download the fix archive, and take all the files that exist within the `ShaderFixes` folder:

https://helixmod.blogspot.com/2018/09/unity-universal-fix.html

_I do not claim anything with this, I just wanted something to easily copy and paste when trying new games that is in the current format of the d3dx.ini that's in new geo-11 builds_
...
; A custom shader to implement a help text overlay.
include = ShaderFixes\help_text\help.ini
include=ShaderFixes\Unity_2019_1.ini
...
; because writing down the hash from the screen is a bad habit that tends to
...
; large amount of space, and costs performance, so disabled by default.
...
; Uncomment to automatically disable the broken Epic Overlay which often causes crashes.
;epic_games_overlay = 0
...
; It has three options, allow_create_device=1, which will allow anything to go through
; without returning an error, and allow_create_device=2, which will force all requests
; to become 11.0 D3D11 Devices, no matter what was passed in. allow_create_device=3 will
; force all devices to be created as 11.1. This is an alternate approach that instead of
; focing a game to fallback the API in use, this asks the game to specifically use a 
; known good and required API of 11.1.
...
;
; allow_platform_update now allows for a setting =2, where it will allow any calls
; through without reporting errors.  This is experimental and incomplete, but can
; help for some games. Some games recognize they are on Win10 and expect higher level
; objects to exist.
;
...
; These parameters are also passed as part of CreateSwapChain, and can be 
; modified to override default behavior.  See the swap chain desc for details: 
; https://docs.microsoft.com/en-us/windows/win32/api/dxgi1_2/ns-dxgi1_2-dxgi_swap_chain_desc1
; In general, these should not be changed from what the game wants.
;
; The DXGI_FORMAT for the swapchain being created.  Normally you want this to be
; whatever the game decides, but you might want to override it to remove SRGB
; or make it UNORM. If not defined, the value the game sets is used.
;dxgi_format = 28
;
; MSAA can be specified for the createswapchain, but will only work if the 
; mode is windowed, non flip. If not defined, the value the game sets is used.
;msaa_count = 1
;msaa_quality = 0
;
...
[Resource_InvViewProj_VS]
type = Buffer
format = R32G32B32A32_FLOAT
array = 4

[ResourceInput_InvViewProj_VS]
max_copies_per_frame = 1

[ResourceBackupCB_InvViewProj_VS]
[ResourceBackup_U_InvViewProj_VS]


[CustomShader_InvViewProj_VS]
cs = ShaderFixes/inverse-cs-InvViewProj_VS.hlsl
ResourceBackupCB_InvViewProj_VS= ref cs-cb0
ResourceBackup_U_InvViewProj_VS= ref cs-u0
; Input constant buffer - the matrix to inverse is here:
cs-cb0 = ResourceInput_InvViewProj_VS
; Output buffer - this will hold the result:
cs-u0 = Resource_InvViewProj_VS
Dispatch = 1, 1, 1


[Resource_InvViewProj2_CS]
type = Buffer
format = R32G32B32A32_FLOAT
array = 4

[ResourceInput_InvViewProj2_CS]
max_copies_per_frame = 1

[ResourceBackupCB_InvViewProj2_CS]
[ResourceBackup_U_InvViewProj2_CS]

[CustomShader_InvViewProj2_CS]
cs = ShaderFixes/inverse-cs-InvViewProj2_CS.hlsl
ResourceBackupCB_InvViewProj2_CS= ref cs-cb4
ResourceBackup_U_InvViewProj2_CS= ref cs-u4
; Input constant buffer - the matrix to inverse is here:
cs-cb4 = ResourceInput_InvViewProj2_CS
; Output buffer - this will hold the result:
cs-u4 = Resource_InvViewProj2_CS
Dispatch = 1, 1, 1

[ResourceInput_InvProj_VS]
max_copies_per_frame = 1

[TextureOverride_5008_ShadowsFix]
match_type = Texture2D
match_width = height
match_bind_flags = +depth_stencil
StereoMode = 2

;[ShaderOverride_Parallax_Occ_Map_VS_1]
;Hash=4c9936f7f1d28f6e
;run = CustomShader_InvViewProj_VS
;ResourceInput_InvViewProj_VS=ref vs-cb3
;vs-cb13 = copy Resource_InvViewProj_VS
